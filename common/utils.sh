...
function get_commit_version() {
    json_data=$(curl -sL -H "Accept: application/json" "https://github.com/RioTwWks/$1/commits/main.atom")
...
function get_pre_release_version() {
    ...
    VERSION=$(curl -sL "https://api.github.com/repos/RioTwWks/$1/releases" | jq -r 'map(select(.prerelease == true or .draft == true)) | sort_by(.created_at) | last | .tag_name')
    VERSION=${VERSION/#v/}
    echo $VERSION
}

function get_release_version() {
    VERSION=$(curl -sL "https://api.github.com/repos/RioTwWks/$1/releases" | jq -r 'map(select(.prerelease == false)) | sort_by(.created_at) | last | .tag_name')
    if [ -z $VERSION ]; then
        # COMMIT_URL=https://api.github.com/repos/hiddify/$1/releases/latest
        # VERSION=$(curl -s --connect-timeout 1 $COMMIT_URL | jq -r .tag_name)
        location=$(curl -sI "https://github.com/RioTwWks/$1/releases/latest" | grep -i location | awk -F' ' '{print $2}' | tr -d '\r')
